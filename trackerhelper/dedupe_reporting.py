...
from .dedupe_models import DedupeResult
...
def write_reports(result: DedupeResult, out_dir: Path) -> DedupeReportPaths:
    ...
    if result.unsafe:
        ...
        for r in result.unsafe:
            lines.append(
                f"UNSAFE: {r}  unique_tracks={result.unique_count[r]}  total_tracks={result.sizes[r]}\n"
            )
        lines.append("\n")
    ...
    if not result.redundant:
        ...
        dups = sorted([r for r in result.redundant if r in result.duplicate_of])
        subs = sorted([r for r in result.redundant if r not in result.duplicate_of])
        ...
        if dups:
            ...
            for r in dups:
                lines.append(
                    f"DELETE: {r}\n"
                    f"  identical_to: {result.duplicate_of[r]}\n"
        ...
        if subs:
            ...
            for r in subs:
                ...
                lines.append(
                    f"DELETE: {r}\n"
                    f"  contained_in: {c}\n"
                    f"  tracks: {result.sizes[r]} -> {result.sizes.get(c, '?')}\n"
